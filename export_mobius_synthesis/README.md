...
The limits are compiled alongside, which is what makes the layer usable. The tensor theorem does not permit an arbitrary function of the same $11^2$ coordinate, and even on a retained cell the selected-prime projection carries only the selected sign while the Mertens-visible observable carries the parity of every prime factor. Separately, the maximal-wheel blocker forces every complete outside least-square owner below $11$, leaving exactly $\{3,5,7\}$, with genuinely summable owner bounds $\lvert\chi^T m_q\rvert\le 18K/q^2$.
