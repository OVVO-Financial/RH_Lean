...
### PR #265 — survivor residue covariance

- [x] `RHLean/Proof/SurvivorResidueCovariance.lean` merged and imported.
- [x] Active survivor prime fibres are partitioned by signed height residue `q^2-c^2 mod s`.
- [x] Signed residue masses `A_{t,s}(u)`, total energy `V`, diagonal `D`, cross covariance `C`, and the explicit unequal-cofactor Gram ledger are machine checked.
- [x] Exact `V = D + C` and finite-torus Parseval identities are compiled.
- [x] No analytic covariance estimate, independence assumption, or unsigned sparsity claim is introduced.

### PR #266 — survivor covariance criterion

- [x] Preceding dependency PR #265 confirmed merged and green.
- [x] Branch `agent/survivor-covariance-criterion` created from current `main` at merge commit `4e62b0dcf6c55cc5d23be09e7e5fcec3137536a9`.
...
- [x] Exploratory CI run `31271219185` on head `15bf8d20280021c3a11dceb16b363e7cdd155585` passed the paper/Analysis boundary check, assumption audit, generated-root check, and `lake build RHLean --wfail` with no warnings before this closeout commit.
...
- [ ] Next exact dependency: reindex the cross covariance as a Möbius-signed active source-pair collision count before any analytic estimate.
