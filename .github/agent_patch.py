...
axer = Path('RHLean/Analysis/NativePNTAxer.lean')
axer.write_text(r'''import Mathlib
...
theorem nativeMertens_div_atTop_zero :
  ...
  · intro b hb
    ...
    have hrem :
      ...
      have hnum : 0 ≤ alpha + D + 2 := by linarith
      have hcross := (div_lt_iff₀ hlogpos).2 ?_
      · exact hcross
...
root = Path('RHLean.lean')
s = root.read_text()
needle = 'import RHLean.Analysis.NativePNTChebyshev\n'
assert needle in s
assert 'import RHLean.Analysis.NativePNTAxer\n' not in s
s = s.replace(needle, 'import RHLean.Analysis.NativePNTAxer\n' + needle, 1)
root.write_text(s)
