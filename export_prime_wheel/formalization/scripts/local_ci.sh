...
# Local mirror of the hosted "Lean verification" job for this project.
#
...
# .github/workflows/lean.yml in the published repository.
#
...
lean_lib='RHLean'
axiom_import='RHLean.Analysis.PublicPrimeWheelReduction'
axiom_decl='RHLean.Analysis.primorialWheel_dirichletNonconcentration_iff_mertensEnergy'

# The hosted job runs no anchor assertion for this export, so this list is
# empty. Add 'label=extended regex' entries here if one is ever added there.
anchor_checks=()
...
step 'Printing the public reduction and its axioms'
cat > "$check_lean" <<EOF
import $axiom_import
...
step 'Asserting the public reduction rests only on the standard Lean axioms'
