...
The terminal theorem and the deterministic shell bridge now both live in `RHLean/Proof/SignedTransportAmplificationAudit.lean`, so the normal rooted `lake build RHLean` kernel-checks the entire terminal layer.

The rounded recurrence target is

`M(X_R)^2 <= C R^2 K + 4 * sum_q M(s_q^2 - 1)^2`

for every lower critical envelope `K` below `R`. Because `s_q^2 <= Y_q`, the sharp odd-owner daughter budget gives
...
The literal-to-rounded daughter shell uses only the trivial interval variation of Mertens, deterministic square-anchor geometry, and Young's inequality:
