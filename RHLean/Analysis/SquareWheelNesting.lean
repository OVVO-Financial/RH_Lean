...
of complete square blocks both lie strictly after the lower primorial endpoint
and at or before the upper endpoint, then the run is exactly the difference of
the two pinned primorial-wheel residual values. -/
theorem sum_canonicalTotalIncrement_Ico_eq_primorialResidual_sub
  ...
  rw [primorialWheel_residual_cast_eq_mertens_sub
      k hrightLower hrightUpper,
    primorialWheel_residual_cast_eq_mertens_sub
