...
private theorem canonicalCoordinates_mul_of_sourceData
  ...
  have hcpos : 0 < c := Nat.zero_lt_of_lt hdata.2.1
  have hqle : q ≤ c * q := by
    calc
      q = 1 * q := by simp
  ...
  constructor
  · simpa [s, sourcePrime, sourceProduct, Nat.mul_comm] using hp.symm
  · simpa [s, sourceCore, sourceProduct, Nat.mul_comm] using hc.symm
...
private theorem moebius_mul_eq_neg_of_sourceData
  ...
  have hmu :=
    ArithmeticFunction.isMultiplicative_moebius.map_mul_of_coprime
      hdata.2.2.2.1
  rw [hmu, ArithmeticFunction.moebius_apply_prime hdata.1]
  ring
...
def primorialMinimalWheelEndpointCofactorSet (k q : ℕ) : Finset ℕ :=
  (Finset.Icc 1 (primorialBlockUpper k)).filter fun c =>
...
theorem endpointSquarefreePrimeMass_eq_endpointCofactorFiber
  ...
  · intro m₁ hm₁ m₂ hm₂ hcof
    rcases Finset.mem_filter.mp hm₁ with ⟨hm₁Ioc, hsq₁, hpq₁⟩
    rcases Finset.mem_filter.mp hm₂ with ⟨hm₂Ioc, hsq₂, hpq₂⟩
    have hLowerPos : 0 < primorialBlockLower k := primorialEndpoint_pos k
