...
theorem nativeHarmonic_succ_sub_log_bounds
  ...
  have hratio : ((N + 1 : ℕ) : ℝ) ≤ 2 * (N : ℝ) := by
    push_cast
    nlinarith
  have hlogratio : Real.log (N + 1 : ℝ) ≤ Real.log 2 + Real.log N := by
    calc
      Real.log (N + 1 : ℝ) ≤ Real.log (2 * (N : ℝ)) := by
        apply Real.log_le_log
        · positivity
        · exact hratio
...
theorem nativePNTWeightedErrorMass_abs_le
  ...
  · have hN : 1 ≤ N := hNpos
    ...
    have hfrac0 : 0 ≤ nativePsi N / (N + 1 : ℝ) := div_nonneg hpsi0 hden.le
    have hfrac : nativePsi N / (N + 1 : ℝ) ≤ Real.log 4 + 2 := by
      have hNlt : (N : ℝ) < (N + 1 : ℕ) := by exact_mod_cast Nat.lt_succ_self N
      have hconst0 : 0 ≤ Real.log 4 + 2 := by
        have := Real.log_nonneg (show (1 : ℝ) ≤ 4 by norm_num)
        linarith
      calc
        nativePsi N / (N + 1 : ℝ) ≤
            ((Real.log 4 + 2) * (N : ℝ)) / (N + 1 : ℝ) :=
