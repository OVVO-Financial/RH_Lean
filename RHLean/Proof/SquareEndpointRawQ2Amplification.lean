...
This leaf module closes the deterministic literal-to-rounded q² shell and
records the exact logical status of the resulting recurrence.

A literal factor-four odd-owner q² recurrence at square endpoints implies the
fixed endpoint amplification statement.  The only losses are the already
proved unsigned shell

`M(Y_q)^2 <= (37/36) M(s_q^2-1)^2 + 148 (s_q+1)^2`

and the sharp owner-scale budget.  Conversely, fixed endpoint amplification
implies the existence of such a raw recurrence constant by elementary padding.
Thus the raw recurrence is not a weaker bookkeeping target: up to absolute
constants it is exactly the endpoint-amplification difficulty.
-/
...
constant, and the q² child energy is nonnegative. -/
