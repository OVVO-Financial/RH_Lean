...
The previous module puts the #683 partner column on the reciprocal compression
carrier and proves the exact scaled Stokes identity.  This file compares the
two exact evolutions of the same current state:
...
Subtracting the two step identities therefore gives the exact memory law
...
`1 - 1/p`.  This is the finite physical identity needed if the post-root
partner boundary is to telescope through the earlier sub-root prime chronology.
