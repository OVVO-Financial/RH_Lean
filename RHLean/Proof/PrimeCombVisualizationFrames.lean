...
The later sections identify the two geometric thresholds shown by the
animation:
...
theorem primeCombFrameSite_empty_of_two_le
  ...
  simp [primeCombFrameSite, PrimeCombFrameSquareHit, hn.ne, hn.ne']
...
@[simp] theorem primeCombFrameDivisors_insert
  ...
  ext q
  by_cases hpn : p ∣ n
  · simp [primeCombFrameDivisors, hpS, hpn]
  · simp [primeCombFrameDivisors, hpS, hpn]
...
theorem primeCombSqrtFrame_tailSeat_eq_cofactorMoebius
  ...
  have hnpos : 0 < c * p := by positivity
  have hprime : ∀ q ∈ primesUpTo (Nat.sqrt W), Nat.Prime q := by
    intro q hq
    exact prime_of_mem_primesUpTo hq
  have htouched := primeCombSqrtFrame_tailSeat_touched hp hpRoot hc hcpW
  have hframe := primeCombFrameSite_eq_allPlus_of_squarefree_touched
    (primesUpTo (Nat.sqrt W)) hprime hnpos (by omega) hsq htouched
...
theorem primeCombSqrtFrame_largePrimeSeat_eq_neg_one
  ...
  have hdiv : primeCombFrameDivisors (primesUpTo (Nat.sqrt W)) p = ∅ := by
    ext q
    simp only [primeCombFrameDivisors, Finset.mem_filter, Finset.not_mem_empty,
      iff_false]
    intro h
