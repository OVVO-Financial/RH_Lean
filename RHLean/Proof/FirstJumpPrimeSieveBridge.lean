...
After the tail-empty contraction, the first-jump residual is a literal high-prime
Mertens band.  The finite quotient reindex below is unconditional: root equality
matters only if a later theorem invokes the separate high-source interpretation,
not for this coordinate conversion.
-/
