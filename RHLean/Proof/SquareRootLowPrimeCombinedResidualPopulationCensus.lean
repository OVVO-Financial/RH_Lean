...
/-!
# Six finite population identities for the combined residual

This file is the population census for #646.  It deliberately contains no norm,
energy inequality, Mertens estimate, or new ledger remainder.  All populations
are compared before summation.

The common tagged currency separates the five roles that can survive the exact
coordinate changes.  `seed` is the saturated `SourceIndex` language.  The three
boundary constructors are literal root/terminal/near populations.  `stableFar`
is kept separate until the final finite filter decides whether it is already a
RoughPrefix daughter or a genuine unclassified far atom.
-/
