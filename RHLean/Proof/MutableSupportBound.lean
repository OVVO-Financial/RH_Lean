...
program.  If the Möbius mass on the settled interior of a square block is exactly
zero, then the whole block discrepancy is supported on the remaining mutable set.
Since every Möbius value has absolute value at most one, the mutable set can change
...
/-- Every Möbius value has integer absolute value at most one. -/
theorem abs_moebius_le_one (m : ℕ) : |μ m| ≤ (1 : ℤ) := by
  rcases ArithmeticFunction.moebius_eq_or m with h | h | h <;> simp [h]

/-- The absolute Möbius mass of any finite set is bounded by its cardinality. -/
