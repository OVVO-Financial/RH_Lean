...
/-!
# Reassemble the frozen top image with the external terminal closure

The global top/bottom cancellation gives the exact endpoint form

`D_R = U_R + Terminal_R - TopImage_R`.

The terminal boundary itself has two genuinely different pieces.  The internal
piece has pivot `p <= R`; the external piece has `R < p` and is literally the
repeated-parent part of the existing external high-prime grid.  This file makes
that latter identification at the level of the complete signed sum and then
substitutes the already-proved external closure
...
Thus the frozen-top route does consume the repeated external high-prime mass
before any norm, but it does not permit the top image itself to be identified
with that grid: every top-image pivot is strictly below `R`, whereas every
external terminal pivot is strictly above `R`.

No PNT input, Mertens estimate, or RH-critical hypothesis is introduced.
-/
...
literally disjoint.  Any cancellation between them must pass through another
physical coordinate; it cannot be a one-step carrier inclusion. -/
...
private theorem externalTerminalTag_mem_repeatedExternal
  ...
  have hdown := squareRootExternalTerminalTag_mem_downcross hR hzBase
  have htFiltered :
...
private theorem repeatedExternal_to_faceCarrier
  ...
  have hgt := (Finset.mem_filter.mp hy).2
...
/-- **Exact frozen-top/external normal form.**  The external part of the
terminal boundary has now been identified with the already-existing `ERrep`
ledger, while the top image stays as its distinct sub-root-pivot population. -/
...
/-- The one signed residual left after combining the frozen top image with the
external high-prime closure. -/
def lowWheelFrozenTopFarResidual (R : ℕ) : ℂ :=
  lowWheelCanonicalRepeatedTerminalInternalLedger R -
    lowWheelFrozenCofactorTopImageLedger R -
    survivorSixteenFarUpperPrimeMass (R - 1)

/-- **Exact reassembly with the existing external closure.**  For `R >= 56`,
the complete canonical defect is the single frozen/top/far residual, plus only
three already-root-scale ledgers. -/
