...
/-!
# Reindex the matched-core middle term to source integers

The Mertens obstruction module leaves the exact correlated term

`Middle(R) = PositiveSmooth(R) - NearPrimeTransport(R)`.

This file makes that term a literal finite source population.  The already
compiled positive part consists of canonical integers `m = c*q` with

`q = P+(m) <= R` and `c < q`.

The near-root transport contributes the next seven possible largest-prime
coordinates `R < q <= R+7`.  Reindexing those fibres by `m = c*q` and using the
same canonical factorization shows that their source mass is exactly the
negative near-prime transport.  Consequently

`Middle(R)`

is the Möbius mass of the single source set

`1 <= m <= R^2-1`, `P+(m) <= R+7`, `canonicalCofactor(m) < P+(m)`.

For `m > 1`, the last inequality is the exact integer orientation
`m < P+(m)^2`, i.e. the canonical form of `sqrt(m) < P+(m)`.

No estimate, absolute value, prime-density input, Mertens hypothesis, or RH
input is used.
-/
...
private theorem nearPositivePair_surjective
  ...
  · apply Finset.mem_filter.mpr
    ...
    · simpa only [hlargest, hcofactor] using And.intro hRq (And.intro hqUpper hcq)
