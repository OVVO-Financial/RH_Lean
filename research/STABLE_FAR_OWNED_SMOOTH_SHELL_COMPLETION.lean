...
/-!
# Completion: the owned terminal carrier is the full smooth square shell

The internal-terminal saturation theorem removes the only missing converse.
Every squarefree child in the physical square shell has an ordered Euler cut.
If its high cofactor is nontrivial, the cut is automatically in the repeated
frozen-cofactor source.  If its high cofactor is one and the whole child is
`R`-smooth, the pivot is at most `R`, hence the cut is in the repeated internal
terminal source.  Thus the two owned images are exactly the complete squarefree
`R`-smooth shell above the root.
-/
...
/-- **Owned terminal saturation of the whole smooth shell.**

The arithmetic home carrier of the internal-terminal mate plus frozen-cofactor
top image is not a special subset: it is exactly every squarefree `R`-smooth
integer in the open square shell `(R,R^2)`. -/
