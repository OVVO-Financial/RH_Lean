...
The first missing converse is purely finite.  An ordered Euler cut with high
cofactor one and pivot still inside the inclusive low wheel cannot be a
unique-parent occurrence at the square endpoint: an explicit second pivot gives
another downcross with the same root-side parent.  This saturates the internal
terminal source before identifying the owned-product image with the complete
squarefree `R`-smooth shell.
-/
...
/-- Build a second terminal downcross with the same low face product.  No
ordering relation between the new pivot and the old face is required here: we
only need membership in the full tagged downcross carrier. -/
...
parent fibre.  Hence it belongs to the repository's internal repeated-terminal
population. -/
