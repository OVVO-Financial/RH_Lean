...
This scratch contains only the exact signed algebra.  It deliberately leaves
the finite counting estimate to a separate layer, so the wheel identity itself
can be kernel-checked without the large floor-arithmetic proof.
-/
...
theorem roughMertens_one_eq_2310Wheel_twoOverlapsCancelled_scratch (B : ℕ) :
    roughMertens 1 B =
