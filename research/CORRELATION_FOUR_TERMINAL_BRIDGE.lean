...
This file consumes the general fixed-coefficient q^2 correlation interface
compiled in `CANONICAL_ROUGH_Q2_TAIL_REDUCTION`.  The hard input is the
factor-four case.  Two root-scale corrections are then absorbed with one
percent Young slack each.  The resulting literal q^2 coefficient is
...
No analytic estimate is proved here.  This is deterministic closure wiring:
LOW-4 is equivalent to global CORR-4 through the q^2-tail localization theorem,
and CORR-4 is sufficient for the existing RH terminal engine.
-/
...
/-- Factor-four instance of the general global q^2 correlation estimate. -/
def CanonicalRoughCorrelationFourQ2EnergyStatement : Prop :=
  ∃ C : ℝ, 0 ≤ C ∧ CanonicalRoughCorrelationQ2EnergyStatementWith 4 C

/-- By the q^2-tail theorem from #706, CORR-4 is exactly the same fixed
coefficient problem on the genuinely recursive low-owner sector `q^2 < R`;
only the root-scale boundary constant changes. -/
theorem correlationFour_iff_exists_lowQ2Energy :
    CanonicalRoughCorrelationFourQ2EnergyStatement ↔
      (∃ C : ℝ, 0 ≤ C ∧
        CanonicalRoughCorrelationLowQ2EnergyStatementWith 4 C) := by
  unfold CanonicalRoughCorrelationFourQ2EnergyStatement
  exact exists_fullQ2Energy_iff_exists_lowQ2Energy (A := 4) (by norm_num)
...
private theorem farFourOddQ2DaughterEnergy_nonneg_corrFour (R : ℕ) :
...
theorem correlationFour_implies_rawQ2_10201_over_2500
  ...
  refine ⟨C, hC, ?_⟩
  intro R K hR hK
  let Q : ℝ := farFourOddQ2DaughterEnergy R
  have hQ : 0 ≤ Q := by
    dsimp [Q]
    exact farFourOddQ2DaughterEnergy_nonneg_corrFour R
  have hK1 : 1 ≤ K := one_le_lowerEnvelope_corrFour (by omega) hK
