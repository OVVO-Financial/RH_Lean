...
/-! ## Exact second-owner terminal evaluation -/

private theorem one_mem_secondPrime_emptyBase
    {R : ℕ} (hR : 56 ≤ R) :
    1 ∈ lowOwnerFirstOwnerBaseFiber R (secondPrime R hR) ∅ := by
  have hsPrime := secondPrime_prime hR
  have hX1 : 1 ≤ squareRootEndpoint R := by
    unfold squareRootEndpoint
    have hsq : 2 ≤ R ^ 2 := by nlinarith
    omega
...
private theorem top_mem_secondPrime_emptyBase
  ...
  let s := secondPrime R hR
  let t := topPrime R hR
  have hsPrime : s.Prime := by simpa [s] using secondPrime_prime hR
  have htPrime : t.Prime := by simpa [t] using topPrime_prime hR
  have hst : s < t := by simpa [s, t] using secondPrime_lt_topPrime hR
  have htMem : t ∈ primesUpTo (squareRootEndpoint R) := by
    simpa [t] using topPrime_mem hR
  have htX : t ≤ squareRootEndpoint R := (mem_primesUpTo.mp htMem).2
  have hsig : squarefreeLowerPrimeSignature s t = ∅ := by
    have h :=
      squarefreeLowerPrimeSignature_mul_larger_prime
        (p := s) (r := t) (a := 1) htPrime hst (by norm_num)
  ...
  have hsNotDvd : ¬ s ∣ t := by
    intro hdiv
    have heq : s = t := (Nat.prime_dvd_prime_iff_eq hsPrime htPrime).mp hdiv
    exact (ne_of_lt hst) heq
  unfold lowOwnerFirstOwnerBaseFiber
  apply Finset.mem_filter.mpr
  constructor
...
private theorem secondPrime_topInterior_eq_pair
    {R : ℕ} (hR : 56 ≤ R) :
    primeInteriorPart (topPrime R hR)
        (lowOwnerFirstOwnerBaseFiber R (secondPrime R hR) ∅) =
      ({1, topPrime R hR} : Finset ℕ) := by
  let t := topPrime R hR
  apply Finset.Subset.antisymm
  · simpa [t] using (secondPrime_topInterior_subset_pair (R := R) hR (sig := ∅))
  · intro n hn
    have h1Base :
        1 ∈ lowOwnerFirstOwnerBaseFiber R (secondPrime R hR) ∅ :=
      one_mem_secondPrime_emptyBase hR
    have htBase :
        t ∈ lowOwnerFirstOwnerBaseFiber R (secondPrime R hR) ∅ := by
      simpa [t] using top_mem_secondPrime_emptyBase hR
    have htPrime : t.Prime := by simpa [t] using topPrime_prime hR
    have htNotDvdOne : ¬ t ∣ 1 := by
      intro hdiv
      exact htPrime.ne_one (Nat.dvd_one.mp hdiv)
...
private theorem secondPrime_topInterior_eq_empty_of_sig_ne_empty
    {R : ℕ} (hR : 56 ≤ R) {sig : Finset ℕ} (hsig : sig ≠ ∅) :
    primeInteriorPart (topPrime R hR)
        (lowOwnerFirstOwnerBaseFiber R (secondPrime R hR) sig) = ∅ := by
  let s := secondPrime R hR
...
private theorem reciprocalDaughterWeight_antitone_at_one
    {R n : ℕ} (hn : 1 ≤ n) :
    lowOwnerReciprocalDaughterWeight R n ≤
      lowOwnerReciprocalDaughterWeight R 1 := by
  unfold lowOwnerReciprocalDaughterWeight
  ...
  · have h1Y : 1 ≤ rawQ2ChildCutoff R q := hn.trans hnY
    simp [hnY, h1Y]
  · by_cases h1Y : 1 ≤ rawQ2ChildCutoff R q
    · have hqData := (Finset.mem_sdiff.mp hq).1
      have hqPrime :=
        (mem_primesUpTo.mp (Finset.mem_erase.mp hqData).2).1
      simp [hnY, h1Y]
      positivity
    · simp [hnY, h1Y]
...
private theorem secondPrime_baseCoefficient_le_one_add_H
  ...
  have hsMem : s ∈ primesUpTo (squareRootEndpoint R) := by
    exact (Finset.mem_erase.mp (secondPrime_mem_erase hR)).2
  have hsX : s ≤ squareRootEndpoint R := (mem_primesUpTo.mp hsMem).2
  have hsOne : 1 ≤ s := (mem_primesUpTo.mp hsMem).1.one_le
  have hd := reciprocalDaughterWeight_antitone_at_one (R := R) (n := s) hsOne
  unfold lowOwnerDirichletBaseCoefficient lowOwnerPhysicalDirichletWeight
  rw [if_pos hsX]
  unfold lowOwnerZeroFrequencyMobiusWeight lowOwnerFarTailWeight
  by_cases hRs : R ≤ s
...
private theorem secondPrime_returned_top_eq_zero
    {R : ℕ} (hR : 56 ≤ R) :
    lowOwnerDirichletReturnedCoefficient R (secondPrime R hR) (topPrime R hR) = 0 := by
  let s := secondPrime R hR
  let t := topPrime R hR
  have hsPrime : s.Prime := by simpa [s] using secondPrime_prime hR
  have htTop : squareRootEndpoint R / 2 < t := by
    simpa [t] using endpoint_half_lt_topPrime hR
  have hX2t : squareRootEndpoint R < 2 * t := by omega
  have h2st : 2 * t ≤ s * t := Nat.mul_le_mul_right t hsPrime.two_le
  have hout : squareRootEndpoint R < s * t := hX2t.trans_le h2st
  unfold lowOwnerDirichletReturnedCoefficient lowOwnerPhysicalDirichletWeight
  rw [if_neg (Nat.not_le_of_lt hout)]
...
private theorem secondPrime_empty_baseDifferenceAmplitude
    ...
    lowOwnerStokesSignedAmplitude
        (primeInteriorPart (topPrime R hR)
          (lowOwnerFirstOwnerBaseFiber R (secondPrime R hR) ∅))
        (lowOwnerStokesToggleDifference (topPrime R hR)
...
theorem secondPrime_terminal_eq_two_one_sub_H_mul_L
    {R : ℕ} (hR : 56 ≤ R) :
    (∑ sig ∈ lowOwnerFirstOwnerSignatureSet R (secondPrime R hR),
      lowOwnerFirstOwnerCanonicalStokesTopTerminalBoundary
        R (secondPrime R hR) sig) =
      2 * (1 - lowOwnerReciprocalDaughterWeight R 1) *
        lowOwnerDirichletBaseCoefficient R (secondPrime R hR) := by
  have hsched := secondPrime_schedule_eq_single_top hR
  rw [sum_lowOwnerFirstOwnerTopTerminal_eq_neg_half_interiorDifferenceProduct_of_schedule_single
    hsched]
  have hEmptyMem :
      (∅ : Finset ℕ) ∈ lowOwnerFirstOwnerSignatureSet R (secondPrime R hR) := by
    unfold lowOwnerFirstOwnerSignatureSet
    have h1Base := one_mem_secondPrime_emptyBase hR
    have h1Car := (Finset.mem_filter.mp h1Base).1
    refine Finset.mem_image.mpr ⟨1, h1Car, ?_⟩
    simp [squarefreeLowerPrimeSignature, squarefreePrimeFace]
  rw [Finset.sum_eq_single (∅ : Finset ℕ)]
  · rw [secondPrime_empty_baseDifferenceAmplitude hR,
      secondPrime_empty_returnedDifferenceAmplitude hR]
    ring
...
theorem secondPrime_terminal_le_four
    ...
    (∑ sig ∈ lowOwnerFirstOwnerSignatureSet R (secondPrime R hR),
      lowOwnerFirstOwnerCanonicalStokesTopTerminalBoundary
        R (secondPrime R hR) sig) ≤ 4 := by
  rw [secondPrime_terminal_eq_two_one_sub_H_mul_L hR]
  let H := lowOwnerReciprocalDaughterWeight R 1
  let L := lowOwnerDirichletBaseCoefficient R (secondPrime R hR)
  have hH : 0 ≤ H := by
    dsimp [H]
  ...
  have hLup : L ≤ 1 + H := by
    ...
    exact secondPrime_baseCoefficient_le_one_add_H hR
  by_cases hHone : 1 ≤ H
  · have hnonpos : 2 * (1 - H) * L ≤ 0 := by nlinarith
    linarith
  · have hHlt : H < 1 := lt_of_not_ge hHone
    have hLlt : L < 2 := by linarith
    nlinarith

/-! ## Maximal owner is favorable -/

private theorem topPrime_admittedBase_eq_one
    {R : ℕ} (hR : 56 ≤ R) {sig : Finset ℕ} {a : ℕ}
    (ha : a ∈ lowOwnerFirstOwnerAdmittedBaseFiber R (topPrime R hR) sig) :
    a = 1 := by
  rcases Finset.mem_filter.mp ha with ⟨haBase, hpaX⟩
  have haCar := (Finset.mem_filter.mp haBase).1
  have haPos := (lowOwnerNonzeroMobiusCarrier_squarefree_pos haCar).2
  have htop := endpoint_half_lt_topPrime hR
  have hXlt : squareRootEndpoint R < 2 * topPrime R hR := by omega
  by_contra hne
  have ha2 : 2 ≤ a := by omega
  have hmul :
      2 * topPrime R hR ≤ topPrime R hR * a := by
    simpa [Nat.mul_comm] using Nat.mul_le_mul_right (topPrime R hR) ha2
  omega

private theorem topPrime_emptyBase_eq_one
    {R : ℕ} (hR : 56 ≤ R) {a : ℕ}
    (ha : a ∈ lowOwnerFirstOwnerBaseFiber R (topPrime R hR) ∅) :
    a = 1 := by
  rcases Finset.mem_filter.mp ha with ⟨haCar, hdata⟩
...
private theorem topPrime_emptyReturnedAmplitude_nonneg
    {R : ℕ} (hR : 56 ≤ R) :
    0 ≤ lowOwnerFirstOwnerReturnedChildParentAmplitude R (topPrime R hR) ∅ := by
  unfold lowOwnerFirstOwnerReturnedChildParentAmplitude
  apply Finset.sum_nonneg
  intro a ha
  have ha1 := topPrime_admittedBase_eq_one hR ha
  subst a
  have hw := lowOwnerZeroFrequencyMobiusWeight_nonneg R (topPrime R hR)
  norm_num [RHLean.Analysis.realMoebiusStep] at hw ⊢
  exact hw

private theorem topPrime_returnedAmplitude_eq_zero_of_signature_ne_empty
    {R : ℕ} (hR : 56 ≤ R) {sig : Finset ℕ} (hsig : sig ≠ ∅) :
    lowOwnerFirstOwnerReturnedChildParentAmplitude R (topPrime R hR) sig = 0 := by
  unfold lowOwnerFirstOwnerReturnedChildParentAmplitude
  apply Finset.sum_eq_zero
  intro a ha
  have ha1 := topPrime_admittedBase_eq_one hR ha
  ...
  have hbase := (Finset.mem_filter.mp ha).1
  have hsigOne := (Finset.mem_filter.mp hbase).2.1
  have hone : squarefreeLowerPrimeSignature (topPrime R hR) 1 = ∅ := by
    simp [squarefreeLowerPrimeSignature, squarefreePrimeFace]
  rw [hone] at hsigOne
  exact (hsig hsigOne.symm).elim

theorem topPrime_terminal_nonpos
    {R : ℕ} (hR : 56 ≤ R) :
    (∑ sig ∈ lowOwnerFirstOwnerSignatureSet R (topPrime R hR),
      lowOwnerFirstOwnerCanonicalStokesTopTerminalBoundary
        R (topPrime R hR) sig) ≤ 0 := by
...
private theorem topTerminalOwnerSet_eq_top_two
    {R : ℕ} (hR : 56 ≤ R) :
    topTerminalOwnerSet R = ({topPrime R hR, secondPrime R hR} : Finset ℕ) := by
  apply Finset.Subset.antisymm
  · exact topTerminalOwnerSet_subset_top_two hR
  · intro p hp
    simp only [Finset.mem_insert, Finset.mem_singleton] at hp
    ...
    · subst p
      unfold topTerminalOwnerSet
      apply Finset.mem_filter.mpr
      exact ⟨topPrime_mem hR, by rw [topPrime_schedule_eq_nil hR]; simp⟩
    · subst p
      unfold topTerminalOwnerSet
      apply Finset.mem_filter.mpr
      have hsMem := (Finset.mem_erase.mp (secondPrime_mem_erase hR)).2
      exact ⟨hsMem, by rw [secondPrime_schedule_eq_single_top hR]; simp⟩
...
theorem lowOwnerStokesExceptionalTerminalFrameDomination_one :
  ...
  have htop := topPrime_terminal_nonpos hR
  have hsecond := secondPrime_terminal_le_four hR
  have hframe := root_sq_over_eighteen_le_frameMajorant hR
  have hfour : (4 : ℝ) ≤ (R : ℝ) ^ 2 / 18 := by
    have hRreal : (56 : ℝ) ≤ R := by exact_mod_cast hR
    nlinarith
  calc
    (∑ sig ∈ lowOwnerFirstOwnerSignatureSet R (topPrime R hR),
        lowOwnerFirstOwnerCanonicalStokesTopTerminalBoundary R (topPrime R hR) sig) +
      ∑ sig ∈ lowOwnerFirstOwnerSignatureSet R (secondPrime R hR),
        lowOwnerFirstOwnerCanonicalStokesTopTerminalBoundary R (secondPrime R hR) sig
        ≤ 4 := by linarith
    _ ≤ (R : ℝ) ^ 2 / 18 := hfour
    _ ≤ frameMajorant R hR := hframe
    _ = 1 * frameMajorant R hR := by ring
