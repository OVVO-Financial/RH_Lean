...
Formalization is used here as a mathematical audit: every cutoff, endpoint convention, canonical factor, sign, support condition, and unresolved analytic premise is explicit.  The Lean 4 development is in the repository
\[
 \texttt{https://github.com/OVVO-Financial/RH\_Lean}
\]
and is pinned to mathlib \texttt{v4.24.0}.  The authoritative build command is
\[
 \texttt{lake build RHLean -{}-wfail}.
\]

For this paper there is a dedicated source export at
\[
 \texttt{export\_square\_block/}.
\]
It contains only the square-block manuscript and the Lean source snapshots needed to support its arithmetic, geometry, canonical low/high decomposition, lifetime death estimate, and square-prefix RH bridge.  Modules belonging to other block architectures or to a later synthesis are deliberately excluded from this standalone export.
