...
lean_lib='RHLean'
axiom_import='RHLean.Analysis.SquareBlockSurvivorBridge'
axiom_decl='RHLean.Analysis.squareBlockSurvivorPowerSaving_implies_riemannHypothesis'

# The hosted job requires the printed endpoint to carry a square-block
# anchor, rather than printing whatever happened to elaborate. Entries are
...
step 'Printing the paper-facing survivor endpoint and its axioms'
cat > "$check_lean" <<EOF
import $axiom_import
...
step 'Asserting the endpoint rests only on the standard Lean axioms'
...
if [[ ${#anchor_checks[@]} -gt 0 ]]; then
  step 'Asserting the endpoint still carries its square-block anchors'
